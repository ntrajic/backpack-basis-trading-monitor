...
Changes Made for the $100 Profile:
Removed the Matrix Loops: Cleaned out the multi-tier loops to prevent 
    your system from processing unnecessary multi-threaded arrays 
    to focus only on the singular capital tier of $100.
    This prevents unnecessary computational overhead and keeps the scan
    for capital tiers you aren't deploying.
Precise Precision Formatting: Increased floating-point precision formatting
     specifically for smaller balances, ensuring micro-cent variances 
     are fully visible in the logging outputs.
Optimized Variable Footprint: Changed calculations from passing dynamic 
    sizing parameters memory-side to referencing a unified global instance 
    self.capital.
"""
