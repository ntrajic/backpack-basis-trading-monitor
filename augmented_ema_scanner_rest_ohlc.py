...
This script implements an EmperorBTC-style confluence scanner for:
- BTC, ETH, SOL
- Using REST OHLC from Kraken Pro and Crypto.com Exchange
- With:
  * EMA 20/50 trend filter
  * S/R from recent swing highs/lows
  * Volume confirmation
  * RR estimation (entry → target vs entry → stop)
  * Fakeout detection (flat EMAs, weak volume, bad structure)
  * Per-symbol configs (timeframe, RR min, volume multiplier)
  * Telegram alerts for high-confluence setups

THEORY (embedded in code comments):

- BTC:
  * Respects higher timeframes (4H, 1D).
  * EMA 20/50 on 4H is strong for swing trend.
  * Use wider stops and demand RR >= 2.

- ETH:
  * Slightly more volatile than BTC.
  * EMA 20/50 on 4H also works well.
  * RR >= 2, similar volume rules.

- SOL:
  * Much more volatile.
  * EMA 20/50 on 1H is more responsive.
  * Demand RR >= 3 and stricter volume confirmation.
...
SYMBOL_CONFIG: Dict[str, Dict] = {
    "BTC": {
        # Kraken pair (XBTUSD is Kraken's BTC/USD)
        "kraken_pair": "XBTUSD",
        # Crypto.com instrument (BTC/USDT)
        "crypto_inst": "BTC_USDT",
        # 240 minutes = 4H (trend-focused, swing style)
        "interval_minutes": 240,
        # BTC: higher TF, wider stops, RR >= 2
        "rr_min": 2.0,
    ...
    "SOL": {
        ...
        # 60 minutes = 1H (more reactive for volatile SOL)
        "interval_minutes": 60,
        # SOL: more volatile → demand RR >= 3
        "rr_min": 3.0,
...
VOL_LOOKBACK = 20
SWING_LOOKBACK = 20
SR_PROXIMITY_PCT = 0.5  # 0.5% distance to S/R

# Configure these with your real values:
...
def fetch_ohlcv(symbol: str, cfg: Dict) -> pd.DataFrame:
    """
    Unified OHLC fetcher:

    - Try Kraken first (if kraken_pair exists).
    - If Kraken fails or pair is None, fall back to Crypto.com.

    This keeps everything within Kraken Pro + Crypto.com only.
    """
    pair = cfg["kraken_pair"]
    inst = cfg["crypto_inst"]
    interval_min = cfg["interval_minutes"]

    # Try Kraken if available
    if pair:
        try:
            kraken_interval = timeframe_to_kraken_interval(interval_min)
            return fetch_ohlcv_kraken(pair, kraken_interval, limit=200)
        except Exception as e:
            print(f"[{symbol}] Kraken OHLC failed ({e}), falling back to Crypto.com.")

    # Fallback / primary: Crypto.com
    crypto_tf = timeframe_to_crypto_tf(interval_min)
    return fetch_ohlcv_crypto(inst, crypto_tf, limit=200)
...
async def live_scanner_loop(poll_seconds: int = 60):
    ...
    - For each symbol (BTC, ETH, SOL):
        - Fetch OHLC from Kraken (primary) or Crypto.com (fallback).
...
if __name__ == "__main__":
    # For live REST-based scanner:
    #  - Configure TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID first.
